...
class Cell:
    def __init__(self, n_of_cells):
        self.n_of_cells = n_of_cells

    def __add__(self, other):
        return self.n_of_cells + other.n_of_cells

    def __sub__(self, other):
        if self.n_of_cells > other.n_of_cells:
            return self.n_of_cells - other.n_of_cells
        else:
            print("Ошибка! Отрицательная разность.")

    def __mul__(self, other):
        return Cell(self.n_of_cells * other.n_of_cells)

    def __truediv__(self, other):
        return self.n_of_cells // other.n_of_cells

    def make_order(self, in_row):
        rows, rest = self.n_of_cells // in_row, self.n_of_cells % in_row
        return '\n'.join(['*' * in_row] * rows + (['*' * rest] if rest else []))

    def __str__(self):
        return f'Клетка состоит из {self.n_of_cells} ячеек'
...
# print(cell2 - cell2)
# print(cell1 / cell2)
print((cell1 * cell2).make_order(100))
